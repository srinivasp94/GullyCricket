...
public class EnterScoresHere extends AppCompatActivity {
    int playerOne = 0;
    int balls = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbhandler = new Dbhandler(EnterScoresHere.this);
        Intent intent = this.getIntent();
        Bundle bundle = this.getIntent().getExtras();


        listPlayers = (List<ModelPlayer>) bundle.getSerializable("data");

        for (int i = 0; i < listPlayers.size(); i++) {
            id = listPlayers.get(i).getId();
            name = listPlayers.get(0).getPlayer_name();
            nameNonStricker = listPlayers.get(1).getPlayer_name();
        }
        ...
        scorePlayerOne.setText(" " + listPlayers.get(0).getPlayer_score());
        playerOne = Integer.parseInt(listPlayers.get(0).getPlayer_score());
        txt_score.setText(" " + listPlayers.get(1).getPlayer_score());
        playerNon_Stricker = Integer.parseInt(listPlayers.get(1).getPlayer_score());
    ...
    public void hitWickets(View view) {
        ...
        if (listPlayers.get(1).getPlayer_team().equals(teamnameB)) {
            dbhandler.updateScoreTeamB(new TeamBModel(listPlayers.get(1).getId(), "", Integer.toString(playerNon_Stricker), "", 0, 0, ""));
        } else {
            dbhandler.updateScore(new ModelPlayer(listPlayers.get(1).getId(), "", Integer.toString(playerNon_Stricker), "", 0, 0, ""));
        }
    ...
    private void callIntent(ModelPlayer playerdata, String playerOne) {
        SharedPreferences preferences = getSharedPreferences("myPreferences", Context.MODE_PRIVATE);
        String teamnameA = preferences.getString("TEAMA", null);
        String teamnameB = preferences.getString("TEAMB", null);
...
//        resultScoreintent.putExtra("Balls", balls);
        if (playerdata.getPlayer_team().equals(teamnameB)) {
            dbhandler.updateScoreTeamB(new TeamBModel(playerdata.getId(), "", playerOne, "", 1, 0, ""));
        } else {
            dbhandler.updateScore(new ModelPlayer(playerdata.getId(), "", playerOne, "", 1, 0, ""));
        }
...
//        setResult(Activity.RESULT_OK, resultScoreintent);
    ...
    public void hitWicketstwo(View view) {
        ...
        if (listPlayers.get(0).getPlayer_team().equals(teamnameB)) {
            dbhandler.updateScoreTeamB(new TeamBModel(listPlayers.get(0).getId(), "", Integer.toString(playerOne), "", 0, 0, ""));
        } else {
            dbhandler.updateScore(new ModelPlayer(listPlayers.get(0).getId(), "", Integer.toString(playerOne), "", 0, 0, ""));
        }
